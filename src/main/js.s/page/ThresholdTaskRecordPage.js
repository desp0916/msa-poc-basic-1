 ...
 * @since  Mon May 22 23:03:28 CST 2017
 * @flow
 */
...
const MonitorTaskRecordView = Loadable({
  loader: () => import('../form/MonitorTaskRecordView'),
  loading: Loading,
...
export default class ThresholdTaskRecordPage extends Component {
  ...
  constructor(props) {
    ...
    this.repository = 'thresholdmonitortaskrecords';
    this.pageTitle = '門檻值監控排程通知紀錄';
    this.dialogTitle = '檢視門檻值監控排程通知紀錄';
    ...
    this.searchOptions = [
      ...
      {key: 'findByTaskIdForCurrentUser', value: '門檻值字監控排程ID' },
      // {key: 'findByKwKeywordContainsForCurrentUser', value: '關鍵字' },
    ];

    this.detailColumn = 'id';
    this.detailPath = '/ThresholdTaskRecord/ThresholdTaskRecordView';
  ...
  componentDidMount() {
    ...
    stompClient.register([
			{route: '/topic/newThresholdMonitorTaskRecord', callback: this._handleRefreshList},
			{route: '/topic/updateThresholdMonitorTaskRecord', callback: this._handleRefreshList},
			{route: '/topic/deleteThresholdMonitorTaskRecord', callback: this._handleRefreshList}
    ]);
  }
  ...
  render() {
    ...
    return (
      <div>
        <Switch>
          <Route path={`${this.detailPath}/:itemId`} render={(props) => (<MonitorTaskRecordView {...props} dialogTitle={this.dialogTitle} repository={this.repository} act={Globals.ACT_VIEW} />)}/>
        </Switch>
