 ...
 * @since Mon Mar 6 21:27:51 CST 2017
 * @flow
 */
...
export default class ThresholdTaskForm extends Component {
  ...
  constructor(props) {
    ...
    this.dialogTitles = {};
    this.dialogTitles[Globals.ACT_CREATE] = '新增門檻值監控排程';
    this.dialogTitles[Globals.ACT_UPDATE] = '修改門檻值監控排程';
    this.projects = [];
    this.intervalOptions = APLog.taskIntervals;
    this.twoBoundsOpOptions = [{key: '<', value: '<'}, {key: '<=', value: '<='}];
    this.fields = APLog.fieldsForThresholdMontiorTask;
    this.inputs = {
      details: [ 
              ...
              FetchEntityAssocLink(freshItem, 'project',
                response => {
                  ...
                  FetchURI(freshItem._links.details.href, 
                    response => {
                      // console.log(response.entity._embedded[this.detailsRepository]);
                      this.setState({
                        item: freshItem,
                        isLoading: false,
                        showDialog: true,
  ...
  _handleSubmit(e) {
    ...
    this.inputs.numericField = (this.inputs.numericField1.checked) ? this.inputs.numericField1 : this.inputs.numericField2;
    delete this.inputs.numericField1;
    delete this.inputs.numericField2;

    this.inputs.thresholdAggType = (this.inputs.thresholdBounds.value == 1) ? this.inputs.thresholdAggType1 : this.inputs.thresholdAggType2;
    delete this.thresholdAggType1;
    delete this.thresholdAggType2;
  ...
  render() {
    return (
      <ModalDialog title={this.dialogTitles[this.props.act]} show={this.state.showDialog} onHide={this._handleHide}>
        ...
        <input type="hidden" defaultValue={APLog.THRESHOLD_MONITOR_TASK} ref={ref => this.inputs.type = ref} />
        <input type="hidden" defaultValue="sum" ref={ref => this.inputs.thresholdAggType = ref} />
        ...
        <FormGroup>
          ...
          <FormGroup>
            <Col sm={2}><ControlLabel>數值欄位 *</ControlLabel></Col>
            <Col sm={10}><Radio inline name="numericField" defaultValue="dataCnt" defaultChecked={this.props.act === Globals.ACT_CREATE || this.state.item.numericField === 'dataCnt'} inputRef={ref => this.inputs.numericField1 = ref}>資料筆數（dataCnt 欄位）</Radio></Col>
          </FormGroup>
          <FormGroup>
            <Col smOffset={2} sm={10}><Radio inline name="numericField" defaultValue="procTime" defaultChecked={this.props.act === Globals.ACT_CREATE || this.state.item.numericField === 'procTime'} inputRef={ref => this.inputs.numericField2 = ref}>處理時間（procTime 欄位）</Radio></Col>
          </FormGroup>
          <FormGroup>
            ...
            <Col sm={2}><SelectWithoutLabel options={APLog.thresholdAggTypes} defaultValue={this.state.item.thresholdAggType} inputRef={ref => this.inputs.thresholdAggType1 = ref}/></Col>
            ...
            <Col sm={4}><TextInputWithoutLabel type="text" placeholder="請輸入出現筆數" defaultValue={this.state.item.thresholdOneBound} inputRef={ref => this.inputs.thresholdOneBound = ref} help={this.state.errors.thresholdOneBound} maxLength={10}/></Col>
          </FormGroup>
          <FormGroup>
            ...
            <Col sm={2}><SelectWithoutLabel options={APLog.thresholdAggTypes} defaultValue={this.state.item.thresholdAggType} inputRef={ref => this.inputs.thresholdAggType2 = ref}/></Col>
